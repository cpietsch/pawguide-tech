...
if [[ "${hardware_identity,,}" == *"s100"* ]]; then
  pass "RDK S100 hardware is identified"
else
  fail "device tree and RDK OS metadata do not identify an RDK S100"
fi
...
if [[ "${memory_kib:-0}" -ge 10000000 ]]; then
  pass "at least 10 GB usable RAM detected"
else
  fail "less than 10 GB usable RAM detected; expected an RDK S100/S100P"
fi
...
if [[ -r /etc/pawguide/hardware-profile ]] &&
  [[ "$(< /etc/pawguide/hardware-profile)" == "s100" ]]; then
  pass "installed PawGuide hardware profile is s100"
else
  fail "installed PawGuide hardware profile is not s100"
fi

if ip link show "${wifi_interface}" >/dev/null 2>&1; then
  pass "M.2/USB Wi-Fi interface ${wifi_interface} exists"
else
  fail "Wi-Fi interface ${wifi_interface} does not exist; the S100 needs a supported Wi-Fi module for Go2 AP mode"
fi
...
if [[ -z "${default_route}" ]]; then
  ...
  fail "Go2 Wi-Fi is the default route; Pixel USB should own it"
else
...
if [[ -n "${uplink_interface}" ]]; then
  ...
  warn "default route uses ${default_interface:-unknown}; confirm this is the intended Pixel or travel-router uplink"
fi
...
if [[ "${temperature_sensor_count}" -ge 5 ]]; then
  pass "at least five S100 temperature sensors are exposed"
else
  warn "found ${temperature_sensor_count} temperature sensors; expected five S100 chip sensors"
fi

if systemctl is-active --quiet display-manager.service; then
  warn "desktop display manager is active; the headless Server image avoids the no-HDMI X11 CPU-rendering issue"
else
  pass "desktop display manager is inactive"
fi
...
warn "software cannot validate the 12-20 V supply, cooling airflow, payload mass or connector retention"
