...
if ! command -v uv >/dev/null 2>&1; then
  echo "uv is missing; run bootstrap-rdk-s100.sh first." >&2
  exit 1
fi
...
echo "PawGuide mock gateway installed."
echo "It is fail-closed; the packaged real adapter remains double-gated and disabled."
echo "Run: sudo ${script_dir}/check-s100-readiness.sh"
