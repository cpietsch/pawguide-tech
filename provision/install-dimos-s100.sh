...
for command_name in git uv; do
  if ! command -v "${command_name}" >/dev/null 2>&1; then
    echo "${command_name} is missing; run bootstrap-rdk-s100.sh first." >&2
    exit 1
  fi
done
...
install \
  ...
  -g root \
  -m 0755 \
  "${script_dir}/run-dimos-s100.sh" \
  /opt/pawguide/bin/run-dimos-s100.sh
install \
  -o root \
  -g root \
  -m 0755 \
...
install \
  ...
  -g root \
  -m 0755 \
  "${script_dir}/diagnose-dimos-s100.sh" \
  /opt/pawguide/bin/diagnose-dimos-s100.sh
install \
  -o root \
  -g root \
  -m 0755 \
...
echo
echo "Pinned PawGuide DimOS runtime installed."
